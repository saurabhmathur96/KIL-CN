...
class Leaf(Node):
  ...
  alpha: float 
  r: list
  clt: cython.declare(ChowLiuTree, visibility = "public")
  ...
  def __init__(self, scope, r, C, epsilon, lambda_, alpha):
    ...
    self.C = C2
    # for i, j in zip(*np.nonzero(C2)):
    #  print (f'x{scope[i]} ≻ x{scope[j]}')
  ...
  def fit(self, X):
    ...
    # self.clt.fit_structure(X[:, self.scope])
    # self.clt.fit_parameters(X[:, self.scope])
    # self.X = np.array(X)
...
class InternalNode(Node):
  ...
  values: list[int] 
  weights: np.array 
  children: list[ChowLiuTree] # cython.declare(list[ChowLiuTree], visibility = "public")
  ...
  def penalty(self, C, epsilon):
    ...
    for ii, i in enumerate(scope):
      if i == j or C[j, i] == 0: continue
      # j is monotnoically influenced by i
      # |i| x |j|
      terms = []
...
def weight_conditional_grad(cnet: InternalNode, b, vb):
  # P(A = a| B = b)
  cnet.conditional()
...
@cython.ccall
def learn_cnet(D: Dataset, scope: list, alpha: float, C:np.ndarray, epsilon: float, lambda_: float, min_instances: int):
...
def score(j:int, Xs:list, r:list, scope:list, C:np.ndarray, epsilon:float, lambda_:float, alpha:float):
  ...
  """"
  penalty = 0
  for i in np.flatnonzero(C.T[j]):
    # i is monotonically influenced by j
    if i not in scope: continue
    terms = np.array([
        np.cumsum([np.exp(leaves[vj].logmar([(i, vi)])) for vi in range(r[i])])
        for vj in range(r[j])]).T # |j| x |i|, P(xi <= vi | xj = vj)
    rows = terms[:-1]
    delta = np.array([
        C[i,j]*(row[vj2] - row[vj1]) + epsilon
        for row in rows
        for vj2, vj1 in product(range(r[j]), range(r[j]))
        if vj2 > vj1
    ]) 
    penalty += np.sum((delta > 0).astype(int)*(delta**2))
  
  for i in np.flatnonzero(C[j]):
    if i not in scope: continue
    # j is monotnoically influenced by i
    # |i| x |j|
    terms = []
    for vi in range(r[i]):
      marg = np.exp([leaves[vj].logmar([(i, vi)]) for vj in range(r[j])])
      numer = marg*weights
      term = numer / np.sum(numer)
      terms.append(np.cumsum(term))
    terms = np.array(terms).T
    ...
    delta = np.array([
        C[j,i]*(row[vi2] - row[vi1]) + epsilon
        for row in rows
        for vi2, vi1 in product(range(r[i]), range(r[i]))
        if vi2 > vi1
    ]) 
    penalty += np.sum((delta > 0).astype(int)*(delta**2))
  

  ll = np.array([leaf.loglik(X) for X, leaf in zip(Xs, leaves)])
  total = sum([len(X) for X in Xs])
  return logsumexp(ll, b=weights)/total - lambda_*penalty
  """
